...
import type { HousingListItem } from "@/features/hometax-calc/lib/housingList"
...
// 투자조합출자(그밖의소득공제) 요약(항목수·불일치)+세부(연도/종류별 대조). NTS OUT ↔ YTS 공제액(ΣPEN_SAVE_SUB_AMT).
function buildWorkbook(rows: BatchRow<HousingListItem>[]) {
  const summary = rows.map(({ item, result, error }) => {
    ...
    return {
      CALC_NO: item.calcNo, 이름: item.nm, 총급여: item.totPayAmt,
      항목수: item.lines.length, 불일치: mismatch, 소진: item.exhaustLabel ?? "", 오류: error ?? "",
    }
  })
  const detail = rows.flatMap(({ item, result }) =>
    item.lines.map(line => {
      ...
      return {
        CALC_NO: item.calcNo, 이름: item.nm, 항목: line.label, 코드: line.code,
        전송납입액: line.ytsInput ?? null, YTS공제: line.ytsDdc, NTS공제: ntsVal,
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-investment-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `investment-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

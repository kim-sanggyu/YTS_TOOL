...
import { getEtcCreditItems } from "@/features/hometax-calc/lib/housingList"
...
// 기타세액공제(외국납부·주택차입금이자·납세조합) 요약(항목수·불일치)+세부(항목별 대조) 두 시트. self OUT ↔ YTS RT_* 대조.
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-etccredit-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `etccredit-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

...
import { getHousingSavingsItems, type HousingListItem } from "@/features/hometax-calc/lib/housingList"
...
// 주택마련저축(그밖의소득공제) 요약(항목수·불일치)+세부(항목별 대조). NTS OUT(×40% 한도후) ↔ YTS 공제액(OTO_HOUSE_LOAN_*_AMT).
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-housingsavings-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `housingsavings-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

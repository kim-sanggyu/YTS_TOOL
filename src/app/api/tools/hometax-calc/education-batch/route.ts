...
import { getEducationItems } from "@/features/hometax-calc/lib/housingList"
...
// 교육비 세액공제(소계 8735, ×15%) 요약(항목수·불일치)+세부(항목별 대조) 두 시트. NTS 8735 소계 ↔ YTS RT_EDU_AMT 대조.
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-education-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `education-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

...
import { getInsuranceItems } from "@/features/hometax-calc/lib/housingList"
...
// 보험료 세액공제(보장성 12%·장애인전용 15%) 요약(항목수·불일치)+세부(항목별 대조) 두 시트. self OUT ↔ YTS RT_IF_* 대조.
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-insurance-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `insurance-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

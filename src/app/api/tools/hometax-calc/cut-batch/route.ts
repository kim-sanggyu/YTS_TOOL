...
import { getTaxCutItems } from "@/features/hometax-calc/lib/housingList"
...
// 세액감면(소득세법·조특법30조·조특법30조제외·조세조약) 요약(항목수·불일치)+세부(항목별 대조) 두 시트. self OUT ↔ YTS RT_* 대조.
...
function saveWorkbook(rows: BatchRow<HousingListItem>[], year: string, ntsYear: string): string {
  const dir = path.join(process.cwd(), "data", "hometax-cut-batch")
  fs.mkdirSync(dir, { recursive: true })
  const ts = new Date().toISOString().replace(/[:.]/g, "-")
  const filePath = path.join(dir, `cut-batch-${year}-nts${ntsYear}-${ts}.xlsx`)

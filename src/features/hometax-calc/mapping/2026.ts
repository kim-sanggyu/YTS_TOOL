 ...
 *
 * ▶ 2026 신규 입력코드 백로그(캡처 분석 2026-07-29, ③프로브 후 배선 — 지금은 미배선):
 *   - 8454 : 그밖의소득공제 신규(OUT ≈ ×40% 추정). YTS 원천 미확정.
 *   - 8780 / 8781 : 기부금 878x 신규(8781 한도 10만 추정). YTS 원천 미확정.
 *   - 8745 / 8846 : 기부금 집계(선입선출 이월 크레딧) 코드 — [[reference_nts_gift_carryforward_limit]] 서 발견.
 *   - 8405 : 2025 서 8407 로 단일화한 옛 청약코드(재등장 여부 확인 대상).
 *   ※ 신규 대부분은 결과·집계 에코코드(8901/8917~8925 등, UI가 계산결과 회신)라 입력배선 불필요.
 *   ※ 폐지된 입력코드 없음(안 보인 코드는 상규님 미입력 데이터일 뿐).
 *

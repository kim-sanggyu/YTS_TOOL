 ...
 *   가~아 = 카테고리별 사용액(전송값, useAmt) / 최종공제금액 = YTS 카드소득공제(비교 기준).
 *   ★불변식 주의: "최종공제금액 == OTO_CARD_ETC"는 비소진자에서만 성립(2026-07-12 프로브 표본이 전부
 *     비소진자였던 사각). 소득소진자는 최종공제금액=소진 전 산출값 / OTO_CARD_ETC=소진 후 적용값(0)으로
 *     갈림(2026-07-29 실측: Y202500279 최종공제금액 3,150,795 vs OTO_CARD_ETC 0). 리스트는 OTO_CARD_ETC>0
 *     으로 소진 건을 의도적으로 제외하므로 표시 라인엔 이 갭이 노출되지 않음.
 *
